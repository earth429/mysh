...
int main()
{
    ...
    char **ap, *argv[10], *inputstring; // argvにコマンドを格納するために使う変数群
    FTS *ftsp; 
    FTSENT *p;
    char *dotav[] = {".", NULL};
    char name[BUFSIZ]; // 4つはls-Fのために使う
    ...
    while (fgets(buf, BUFSIZ, stdin) != NULL) { // 入力されたコマンドを一行読む
        ...
        } else if (strcmp(argv[0], "ls-F") == 0) { // ls-Fという内部コマンドの実装(mysh7より追加)
            ftsp = fts_open(dotav, FTS_PHYSICAL, NULL);
            while ((p = fts_read(ftsp)) != NULL) {
                strcpy(name, p->fts_name); // nameにファイルネーム格納(隠しファイル判別のため)
                if (p->fts_level == 1 && name[0] != '.') { // カレントディレクトリのファイルのみ表示する(隠しファイルは表示しない)
                    switch (p->fts_info) {
                        case FTS_F: // 通常のファイルの時
                            if (p->fts_statp->st_mode == 33279) { // よくわからないけど、このとき実行可能ファイル
                                printf("%s*  ", p->fts_name); // 実行可能ファイル
                            } else {
                                printf("%s  ", p->fts_name); // 通常ファイル
                            }
                            break;
                        case FTS_D: // ディレクトリの時
                            printf("%s/  ", p->fts_name);
                            break;
                        case FTS_SL: // シンボリックリンクの時
                            printf("%s@  ", p->fts_name);
                            break;
                    }
                }
